...
\begin{abstract}
Every non-injective function forgets something. We develop a universal
theory of \emph{what} it forgets and \emph{how much} it costs to recover.
...
\texttt{sorry} in proof terms. The theory is discharged against the
...
This paper develops a universal theory of what maps forget.

Every function $\compare : \Real \to \Bare$ between two types identifies
some elements of $\Real$: those that share an image in $\Bare$. When
$\compare$ is injective, nothing is lost. When it is not, the
\emph{residual}---the totality of distinctions that $\compare$ cannot
see---becomes a nontrivial mathematical object.

We prove that the residual is not merely ``some leftover.'' It is a
\emph{canonically organized space of indistinguishability} with a precise
internal geometry, exact observability laws, a resolution theory governed by
graph coloring, and sharp quantitative bounds on the cost of eliminating it.

The theory arose from a certification problem (where $\Bare$ represents
bare certificates and $\Real$ represents realized witnesses), but the
mathematics depends only on the function $\compare$. Group homomorphisms,
topological quotients, type-theoretic projections, measurements, compilers,
and hash functions all inherit the full geometry. We use the language of
certification throughout, but the results are universal.
...
\item See \texttt{MANIFEST.md} for the complete index
\end{itemize}
\end{remark}
