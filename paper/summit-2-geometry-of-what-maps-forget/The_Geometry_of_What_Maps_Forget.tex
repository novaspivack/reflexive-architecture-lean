...
\noindent\textbf{Code availability:} Lean~4 formalization at
\texttt{reflexive-architecture-lean}. Build: \texttt{lake build}.
Zero \texttt{sorry} in proof terms. See \texttt{MANIFEST.md} for the
complete theorem index, \texttt{ARTIFACT.md} for citation and build fingerprint,
\texttt{STRATA\_FORMALIZATION\_MAP.md} for the module gloss, and
\texttt{docs/CONSUMER\_BUILD\_NOTE.md} for consumer-build/linter notes (all at repository root except the last, under \texttt{docs/}).
...
\texttt{sorry} in proof terms on the summit route cited in \texttt{MANIFEST.md}. The theory is discharged against the
...
\textbf{Trust boundary:} Universal claims are theorems in Lean for arbitrary types and comparison maps on the summit route cited in \texttt{MANIFEST.md}; adequacy hypotheses are explicit where enrichment is forced. \emph{Reflexive-closure-lean} supplies a small additional bridge (map forgetting, representation/remainder interfaces, and class-level invariants) indexed in that manifest; the physical reading of conservation as ``law'' remains interpretive (\S\ref{sec:representation-conservation}). Philosophical claims elsewhere in \S\ref{sec:representation-as-structured-forgetting} are interpretive unless noted as restating the formal fiber geometry or those abstract interfaces.
\end{abstract}
...
\item \textbf{Summit route vs.\ partial formalizations.} The abstract cites \texttt{MANIFEST.md} for the summit scope; partial homotopy-layer gaps in other branches do not retroactively weaken this route if not claimed here.
  \emph{Logical effect:} Use the repository's summit index when auditing lemmas.
\end{itemize}
\par}
\end{keybox}
...
\item See \texttt{MANIFEST.md} for the complete index; \texttt{ARTIFACT.md} for citation metadata
\end{itemize}
\end{remark}
...
and the Paper~57 non-exhaustion bridge files cited in \texttt{MANIFEST.md};
see Remark~\ref{rem:structured-forgetting-lean} for names.
...
\noindent
Build: \texttt{lake build} from \texttt{reflexive-architecture-lean/}.
See \texttt{MANIFEST.md} for the complete listing ($\sim$235 theorems, 26 modules),
\texttt{STRATA\_FORMALIZATION\_MAP.md} for module glosses, \texttt{ARTIFACT.md}
for citation and build fingerprint, and \texttt{docs/CONSUMER\_BUILD\_NOTE.md} for consumer-build notes.
